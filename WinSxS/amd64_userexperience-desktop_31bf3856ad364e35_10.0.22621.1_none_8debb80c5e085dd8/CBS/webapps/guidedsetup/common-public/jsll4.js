            ...
            var merge = function(obj)
                {
                    for (var prop in obj)
                    {
                        ...
                        {
                            if (obj[prop] != undefined && deep && Object.prototype.toString.call(obj[prop]) === "[object Object]")
                            {
...
awa.ids = (function()
{
    ...
    function visitorId(allCookies)
    {
        if (allCookies)
        {
            return allCookies["MUID"]
        }
        else
        {
            return cookie.getCookie("MUID")
        }
    }
    ...
    function groups(allCookies)
    {
        if (allCookies)
        {
            return allCookies[expIdCookieName]
        }
        else
        {
    ...
    function muidUserId(allCookies)
    {
        var muidValue = "";
        if (allCookies)
        {
            muidValue = allCookies["MUID"]
        }
        else
        {
            muidValue = cookie.getCookie("MUID")
        }
    ...
    function readExpIdFromCookieAndSetInModule(allCookies)
    {
        if (!allCookies)
        {
            setAppExpId(groups())
        ...
        {
            setAppExpId(allCookies[expIdCookieName])
        }
        return getAppExpId()
    }
    return {
            getVisitorId: visitorId, getMuidUserId: muidUserId, setAppUserId: setAppUserId, getAppUserId: function()
                {
                    return appUserId
                }, readExpIdFromCookie: readExpIdFromCookieAndSetInModule, readExpIdFromCoreData: function(expId)
                {
                    setAppExpId(expId);
                    return getAppExpId()
    ...
    function outgoingRequest(eventData)
    {
        var event = {
                name: "Ms.Webi.OutgoingRequest", cV: eventData.cV, data: {
                        ...
                        }, baseType: "Ms.Qos.OutgoingServiceRequest", impressionGuid: ids.getImpressionGuid(), message: eventData.errorMessage, retryCount: eventData.retryCount, customData: eventData.customData
                    }
            };
...
awa.errorHandler = (function()
{
    ...
    function defaultHandler()
    {
        if (arguments)
        {
            ...
            if (arguments[1])
            {
                scriptSource = awa.sanitizeUrl(arguments[1])
            }
            var errorDetails = {
                    Page: awa.sanitizeUrl(document.location.toString()), Script: scriptSource, Message: arguments[0] || "", LineNumber: arguments[2] || 0, StackTrace: arguments[4] ? extractStackTrace(arguments[4]) : "", UserAgent: awa.utils.getUserAgent() || "", Platform: window.navigator.platform || ""
                };
    ...
    return {
            init: init, testHook: {defaultHandler: defaultHandler}
        }
})();
...
(function()
{
    ...
    var config = {
            endpoint: "https://web.vortex.data.microsoft.com/collect/v1", sendMode: 1, batchSize: 10, authMethod: 0, validateEvents: true, sendEvents: true, logLevel: awa.verbosityLevels.NONE, syncMuid: false, provisionMsfpc: true, useDefaultContentName: true, useShortNameForContentBlob: false, debounceMs: {
                ...
                }, muidDomain: "microsoft.com", biBlobAttributeTag: "data-m", isLoggedIn: false, shareAuthStatus: false, cookiesToCollect: ["Bounced"], mscomCookies: false, userConsentCookieName: "MSCC", userConsented: undefined, useBeacon: true, urlCollectHash: false, urlCollectQuery: true, initCv: false, enabledFeatures: true, ix: {
                    a: false, g: false
                }, autoCapture: {
                    ...
                    referrerUri: undefined, requestUri: "", appId: window.location.hostname, pageName: "", title: "", pageType: "", env: "", product: "", market: "", serverImpressionGuid: "", expId: "", pageTags: {}
                }
        };
    ...
    var allowedGetLength = isOldIE ? 4095 : 15000;
    var queryParameters = "";
    ...
    function getQueryFromJSON(event, allCookies)
    {
        var queryParameters = "";
        ...
        if (config.shareAuthStatus === true && config.authMethod == 2 && awa.getSignedInStatus() === true)
        {
            queryParameters += "&" + getRedirectQueryParam(allCookies)
        }
    ...
    awa.init = function(overrideConfig)
    {
        if (!awa.isInitialized)
        {
            ...
            config.coreData.referrerUri = config.coreData.referrerUri ? config.coreData.referrerUri : awa.sanitizeUrl(document.referrer);
    ...
    var populatePartAandExtensions = function(event, allCookies)
        {
            ...
            var javascriptExtensionObject = {
                    ver: "1.1", libVer: "4.2.15", domain: domain, msfpc: allCookies["MSFPC"], userConsent: isValueAssigned(config.userConsented) ? config.userConsented : (isValueAssigned(allCookies[config.userConsentCookieName]) ? true : false), anid: (allCookies["ANON"]) ? allCookies["ANON"].slice(0, 34) : undefined
                };
            var serviceName = event.serviceName ? event.serviceName : config.coreData.serviceName;
            if (serviceName)
            {
                javascriptExtensionObject.serviceName = serviceName
            }
            if (event.ext)
            {
            ...
            var localId = awa.ids.getMuidUserId(allCookies);
            if (localId)
            {
            ...
            var expIdValue = config.coreData.expId ? awa.ids.readExpIdFromCoreData(config.coreData.expId) : awa.ids.readExpIdFromCookie(allCookies);
    ...
    awa.writeEvent = function(event, doNotRetryOnError)
    {
        if (!awa.isInitialized)
        {
        ...
        var allCookies = awa.cookie.getAllCookies();
        var isMsfpcProvisioned = isValueAssigned(allCookies["MSFPC"]);
        if (Object.prototype.toString.call(event) === "[object Array]")
        {
            var arrayOfEventsToSend = [];
            for (var i = 0; i < event.length; i++)
            {
                var currentEvent = populatePartAandExtensions(event[i], allCookies);
                if (currentEvent)
                {
        ...
        {
            event = populatePartAandExtensions(event, allCookies);
            if (!event)
            {
                return
            }
        ...
        if (config.sendEvents)
        {
            ...
            if (actuallyUseGet)
            {
                if (firstEventSent === false || config.provisionMsfpc && !isMsfpcProvisioned || config.shareAuthStatus === true)
                {
                    ...
                    if (config.shareAuthStatus === true && awa.getSignedInStatus() === true)
                    {
                        if (config.authMethod === 1)
                        {
                            var isAlreadyAuthed = isValueAssigned(allCookies["Bounced"]);
                            if (isAlreadyAuthed)
                            {
                                sendThroughImage(config.endpoint + "/asm.gif?" + getQueryFromJSON(event, allCookies), event);
                                return
                            }
                            else
                            {
                    ...
                    sendThroughScript(config.endpoint + telemetryResource + "?" + getQueryFromJSON(event, allCookies), event, doNotRetryOnError)
                }
                else
                {
                    ...
                    {
                        sendThroughImage(config.endpoint + "/t.gif?" + getQueryFromJSON(event, allCookies), event)
                    }
                }
            }
            else
        ...
        {
            xhrSuccessCallback(getQueryFromJSON(event, allCookies), event)
        }
    };
    ...
    function getRedirectQueryParam(allCookies)
    {
        return "$nr=" + isValueAssigned(allCookies["Bounced"])
    }
    ...
    function sendThroughScript(scriptSrc, event, doNotRetryOnError)
    {
    ...
    awa.sanitizeUrl = function(originalUrl)
    {
        var location = document.createElement("a");
        location.href = originalUrl;
        if (location && originalUrl)
        {
            var sanitizedUrl = location.protocol + "//" + location.hostname + (isValueAssigned(location.port) ? ":" + location.port : "") + location.pathname;
            if (config.urlCollectHash)
            {
                sanitizedUrl += location.hash
            }
    ...
    awa.sendEventThroughIframe = function(frameSrc)
    {
        ...
        frame.id = "telframe" + randomNumber;
...
awa.ct = (function()
{
    ...
    var manualTrackingOnlyFieldName = "data-bi-mto";
    ...
    function init(inputConfig)
    {
        ...
        if (config.authMethod == 2 && config.shareAuthStatus == true && awa.getSignedInStatus())
        {
            if (window.addEventListener)
            {
    ...
    {
        if (event.origin == "https://web.vortex.data.microsoft.com" || event.origin == "https://login.microsoftonline.com")
        {
            if (event.data)
            {
                try
                {
                    ...
                    if (dataObject && dataObject.Action == "firstEventDone")
                    {
                        awa.firstEventDone()
                    }
                    if (dataObject && dataObject.BouncedCookieExpires)
                    {
                        var cookieValue = "Bounced=" + utils.dateToISOString(new Date) + "; expires=" + new Date(dataObject.BouncedCookieExpires).toUTCString() + "; path=/;";
    ...
    function getTitle()
    {
        if (config.coreData.title)
        {
            return config.coreData.title
        }
        else
        {
    ...
    function isGamepadA(evt)
    {
        if (evt.keyCode)
        {
            return (evt.keyCode === 195)
        }
    }
    function isGamepadMenu(evt)
    {
        if (evt.keyCode)
        {
            return (evt.keyCode === 207)
        }
    }
    function processClick(event)
    {
        ...
        var isRightClickEvent = isRightClick(evt);
        ...
        else if (isMiddleClick(evt))
        {
            overrideValues.actionType = actionTypeEnum.CLICKMIDDLE
        }
        else if (isGamepadA(evt))
        {
            overrideValues.actionType = actionTypeEnum.GAMEPADA
        }
        else if (isGamepadMenu(evt))
        {
            overrideValues.actionType = actionTypeEnum.GAMEPADMENU
        }
        else
        {
            return
        ...
        while (e && e.tagName)
        {
            if (e.control && clickCaptureElements[e.control.tagName.toUpperCase()])
            {
                e = e.control
            }
            ...
            else
            {
                ...
                if (sendEvent)
                {
                    if (!isTagFoundInElement(e, manualTrackingOnlyFieldName))
                    {
                        sendClickEvent(e, overrideValues, isRightClickEvent)
                    }
                }
                break
            }
    ...
    function isTagFoundInElement(element, tag)
    {
        var taggedElement = utils.findClosestByAttribute(element, tag);
        if (!utils.isValueAssigned(taggedElement))
        {
            return false
        }
        return true
    ...
    function sendPageView(overrideValues)
    {
        ...
        var allCookies = cookie.getAllCookies();
        var eventData = {};
        ...
        setEnabledFeatures(pageTags, allCookies);
    ...
    function sendContentUpdate(overrideValues)
    {
            ...
            if (autoCapture.perf === true)
            {
                if (window.performance)
                {
                    ...
                    if (window.performance.timing)
                    {
                        ...
                        {
                            var paintTimes = window.performance.getEntriesByType("paint");
                            if (paintTimes)
                            {
                                for (var i = 0; i < paintTimes.length; i++)
                                {
                                    timings[paintTimes[i].name] = paintTimes[i].startTime
                                }
                                for (var attrname in window.performance.timing)
                                {
                                    if (attrname != "toJSON")
                                        timings[attrname] = window.performance.timing[attrname]
                                }
                            }
                            else
                            {
                        ...
                        pageTags.timing = utils.stringifyField("timing", timings)
                    }
        ...
        var contentUpdateEvent = {
                name: "Ms.Webi.ContentUpdate", data: {
                        ...
                        }, baseType: "Ms.Content.ContentUpdate", timings: JSON.stringify(overrideValues.timings), title: overrideValues.title || getTitle(), cookieEnabled: getCookieEnabled(), isJs: true, isManual: true, isDomComplete: false, isLoggedIn: awa.getSignedInStatus(), serverImpressionGuid: eventData.serverImpressionGuid
                    }, flags: eventData.isSensitive ? IS_SENSITIVE_FLAG : undefined
            };
    ...
    function sendClickEvent(element, overrideValues, isRightClick)
    {
        overrideValues = overrideValues || {};
        var isMto = overrideValues.isAuto ? false : isTagFoundInElement(element, manualTrackingOnlyFieldName);
        if (element && isTagFoundInElement(element, doNotTrackFieldName) && !isMto)
        {
        ...
        if (element)
        {
            ...
            if (isRightClick)
            {
                eventData.behavior = awa.behavior.CONTEXTMENU
            }
    ...
    function sendPageUnload(overrideValues)
    {
        if (!isPageUnloadFired)
        {
            ...
            overrideValues = overrideValues || {};
            overrideValues.isDomComplete = true;
            var eventData = {};
            ...
            sendEventAfterOverridesAndStringify(pageUnloadEvent, overrideValues, eventData.env, true)
        }
    }
        ...
        if (overrideValues)
        {
            ...
            var clientErrorEvent = {
                    name: "Ms.Webi.ClientError", data: {
                            errorInfo: JSON.stringify(overrideValues.errorInfo), wasDisplayed: overrideValues.displayedToUser || false, impressionGuid: ids.getImpressionGuid(), pageName: overrideValues.pageName || getPageName(), uri: overrideValues.uri || getCurrentUri(), market: eventData.market, serverImpressionGuid: eventData.serverImpressionGuid
                        }, flags: eventData.isSensitive ? IS_SENSITIVE_FLAG : undefined
                };
            sendEventAfterOverridesAndStringify(clientErrorEvent, overrideValues, eventData.env, false)
        }
    }
        ...
        if (overrideValues)
        {
            ...
            var corsDisallowedEvent = {
                    name: "Ms.Cll.Javascript.CorsDisallowed", data: {
                            eventName: overrideValues.eventName, pageName: getPageName(), uri: overrideValues.uri || getCurrentUri(), market: eventData.market, impressionGuid: ids.getImpressionGuid(), serverImpressionGuid: eventData.serverImpressionGuid
                        }, flags: eventData.isSensitive ? IS_SENSITIVE_FLAG : undefined
                };
            sendEventAfterOverridesAndStringify(corsDisallowedEvent, overrideValues, eventData.env, false)
        }
    }
        ...
        if (overrideValues)
        {
            ...
            var eventTooLongEvent = {
                    name: "Ms.Cll.Javascript.EventTooLong", data: {
                            eventName: overrideValues.eventName, payloadLength: overrideValues.payloadLength, pageName: overrideValues.pageName || getPageName(), uri: overrideValues.uri || getCurrentUri(), market: eventData.market, impressionGuid: ids.getImpressionGuid(), serverImpressionGuid: eventData.serverImpressionGuid
                        }, flags: eventData.isSensitive ? IS_SENSITIVE_FLAG : undefined
                };
            sendEventAfterOverridesAndStringify(eventTooLongEvent, overrideValues, eventData.env, false)
        }
    }
        ...
        if (overrideValues && autoCapture.invalidEvents)
        {
            ...
            var eventTooLongEvent = {
                    name: "Ms.Cll.Javascript.EventDropped", data: {
                            eventName: overrideValues.eventName, droppedInfo: overrideValues.droppedInfo, pageName: overrideValues.pageName || getPageName(), uri: overrideValues.uri || getCurrentUri(), market: eventData.market, impressionGuid: ids.getImpressionGuid(), serverImpressionGuid: eventData.serverImpressionGuid
                        }, flags: eventData.isSensitive ? IS_SENSITIVE_FLAG : undefined
                };
            sendEventAfterOverridesAndStringify(eventTooLongEvent, overrideValues, eventData.env, false)
        }
    ...
    function extendAndSendQos(qosEvent)
    {
        ...
        partC.serverImpressionGuid = eventData.serverImpressionGuid;
        partC.market = eventData.market;
        partC.pageName = getPageName();
        partC.uri = getCurrentUri();
    ...
    function populateOverridesInEvent(event, partC, overrideValues)
    {
        ...
        if (overrideValues.serviceName)
        {
            event.serviceName = overrideValues.serviceName
        }
    ...
    function pushToArrayOfContentsIfVisible(elements, arrayOfContents, viewportBoundingRect)
    {
        if (elements)
        {
            for (var i = 0; i < elements.length; i++)
            {
                ...
                if (!isTagFoundInElement(element, doNotTrackFieldName))
                {
    ...
    function getLineageDetails(element)
    {
        ...
        var lineageDetails = {
                lineage: name.join(lineageDelimiter), lineageById: identifier.join(lineageDelimiter), containerName: containerName
            };
    ...
    function getClientCookies(allCookies)
    {
        var cookies = "";
        var uniqueCookies = {};
        var mergedCookies = [];
    ...
    function setEnabledFeatures(pageTags, allCookies)
    {
        if (config.enabledFeatures)
        {
            ...
            var enabledFeatures = allCookies["Features"];
            if (enabledFeatures)
            {
                ...
                var sendApiComplete = function(jqXhr, isSuccess, timeTaken)
                    {
                        ...
                        var event = {
                                name: "Ms.Webi.OutgoingRequest", cV: cachedCv, data: {
                                        baseData: {
                                            operationName: requestOptions.currentOperationName || awa.sanitizeUrl(document.location.toString()), targetUri: targetUriOverrideCallbackFunction ? targetUriOverrideCallbackFunction(options.url) : options.url, latencyMs: timeTaken, serviceErrorCode: (!isSuccess && jqXhr.responseJSON && jqXhr.responseJSON.code && !isNaN(jqXhr.responseJSON.code)) ? jqXhr.responseJSON.code : -1, succeeded: isSuccess, requestMethod: options.type, responseContentType: options.dataType, protocolStatusCode: jqXhr.status.toString(), dependencyOperationName: operationName, dependencyOperationVersion: requestOptions.version && requestOptions.version.toString(), dependencyName: requestOptions.serviceName, dependencyType: "WebService", responseSizeBytes: contentLength && parseInt(contentLength, 10)
                                        }, baseType: "Ms.Qos.OutgoingServiceRequest", impressionGuid: ids.getImpressionGuid(), message: isSuccess ? undefined : awa.utils.stringifyField("errorMessage", jqXhr.errorThrown), retryCount: retryCount, customData: (requestOptions.customDataCallBack && typeof requestOptions.customDataCallBack === "function") ? JSON.stringify(requestOptions.customDataCallBack(jqXhr)) : undefined
                                    }
                            };
