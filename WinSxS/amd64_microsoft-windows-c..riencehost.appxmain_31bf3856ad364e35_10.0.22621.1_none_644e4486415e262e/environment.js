    ...
    (function (AgeAppropriateDesignCode) {
        ...
        class Eligibility {
            static shouldRestrictionsApplyInRegion(region) {
                ...
                // and %SDXROOT%\onecoreuap\shell\cloudexperiencehost\onecore\app\App\core\js\appLaunchers\OobeSettingsSelector.js,
            ...
            static shouldRestrictionsApplyToAgeUnknown() {
                // Fallback for "Age Unknown" case
                if (CloudExperienceHostAPI.FeatureStaging.isOobeFeatureEnabled("CxhAadcAgeUnknownInScope")) {
                    return (CloudExperienceHostAPI.UtilStaticsCore.getLicensingPolicyValue("AgeAppropriateDesignCode-ApplyToUnknownAgeGroup") === 1) ? true : false;
                }
                else {
                    return false;
                }
            ...
            static shouldRestrictionsApplyToAgeGroupAndRegion(ageGroup, region, shouldRestrictionsApplyToMinorOverStatutoryAge) {
                ...
                switch (ageGroup) {
                    ...
                    default:
                        return Eligibility.shouldRestrictionsApplyToAgeUnknown();
                }
            }
            ...
            static shouldRestrictionsApplyToCurrentUserAsync(shouldRestrictionsApplyToMinorOverStatutoryAge = false) {
                return new WinJS.Promise((completeDispatch /*, errorDispatch, progressDispatch */) => {
                    CloudExperienceHost.MSA.getAccountInformation(null /* userName */, null /* accountId */).then((accountInformationSet) => {
                        ...
                        // Use Windows region as fallback if "country" isn't in the property set
                        let ageGroup = accountInformationSet.hasKey("agegroup") ? parseInt(accountInformationSet.lookup("agegroup")) : AgeGroup.Unknown;
                        let region = accountInformationSet.hasKey("country") ? accountInformationSet.lookup("country").toUpperCase() : CloudExperienceHost.Globalization.GeographicRegion.getCode();
