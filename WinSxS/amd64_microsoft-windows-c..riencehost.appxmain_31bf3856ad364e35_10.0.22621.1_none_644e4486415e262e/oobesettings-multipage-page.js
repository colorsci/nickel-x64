    ...
    WinJS.UI.Pages.define("/webapps/inclusiveOobe/view/oobesettings-multipage-main.html", {
        init: (element, options) => {
            ...
            let getLocalizedStringsPromise = requireAsync(['legacy/bridge']).then((result) => {
                return result.legacy_bridge.invoke("CloudExperienceHost.StringResources.makeResourceObject", "oobeSettingsMultiPage");
            }).then((result) => {
        ...
        ready: (element, options) => {
            require(['lib/knockout', 'corejs/knockouthelpers', 'legacy/bridge', 'legacy/events', 'oobesettings-multipage-vm', 'oobesettings-data', 'lib/knockout-winjs'], (ko, KoHelpers, bridge, constants, SettingsMultiPageViewModel) => {
                ...
                let vm = new SettingsMultiPageViewModel(this.resourceStrings, this.isInternetAvailable, this.targetPersonality);
                ko.applyBindings(vm);
